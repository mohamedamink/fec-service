...
class Related extends React.Component {
  ...
  getProductById= async(id /**11020 */)=> {
    const   product_res = await axios.get('/products/'+ id) /**getting product information by id */
    let product =product_res.data

    const   product_stl_res = await axios.get('/products/'+id+'/styles') /**getting firdt photo from style by id product */
    product.img = product_stl_res.data.results[0].photos[0].thumbnail_url /**adding img to the current product */
    
    //console.log (product_stl_res.data.results[0].sale_price)
    product.sale_price = product_stl_res.data.results[0].sale_price

    const   product_rating_res = await axios.get('/reviews/meta/'+id ) /**getting rattings reviews  */
    product.rating=  this.averageRating(product_rating_res.data.ratings) /**calculating Average rating and addint it to the current product */
  ...
  deleteProductFromOutFIT(f_product_id) {
    ...
    var filtered = newOutFitProduct.filter(product => (product.id !== f_product_id));
  ...
  addProductToOutFit() {
    ...
    const newOutFitProduct = this.state.OutFITProducts
       ...
       const indexFind = this.state.OutFITProducts.findIndex(product => (product.id === this.state.currentProduct.id))
  ...
  productStyles() {
    axios.get('/products/'+this.state.product_id+'/styles').then((response) => {
      //res.send(response.data)
  ...
  showCompairing(f_clicked_product) {
    ...
    Object.keys(this.state.currentProduct.characteristics).map((charc) => {
      ...
      const indexFind = arraycomp.findIndex(compObj => (compObj.currPrd === false && compObj.charc === charc &&  compObj.clickedPrd === true))
      console.log(indexFind)
      if(indexFind ===-1){
      ...
      arraycomp[indexFind].currPrd=true
    ...
    return(
      ...
      // <div style={{ overflowX: 'auto', display:'flex'}} >
       <div   > 
        ...
        <> <div onClick={() => this.setState({showModalCompairing: false})}
        ...
        <tbody><tr className="text-left border-b-2 border-gray-300">
            ...
            <th className="px-4 py-3">Clicked Product</th>
                    ...
                    {this.state.comparingArray.map((compLine, index) => {
                      return(
                        <tr key={index}>
                              <td >{compLine.currPrd && <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                                </svg>}</td>
                  
                              <td style={{textAlign:'center'}}>{compLine.charc}</td>

                              <td style = {{right: '10%', position: 'absolute'}}
>{compLine.clickedPrd && <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6  " fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                              </svg>}</td>
                        </tr>
                      )
          ...
          <div className="opacity-25 fixed inset-0 z-40 bg-black">kgufcvgvh</div> </>}
        {this.state.busy==true ? <div>Loading .....</div>:<div >
          <div> RELATED PRODUCTS</div>
       <Carousel>
    {
          this.state.relatedProducts.map((prd) => {
             return(
               <div key={prd.id} onClick={() => this.showCompairing(prd)}>
       ...
       <Carousel>
       <div key={-1} >
       <Card addProductToOutFit={this.addProductToOutFit}       isButton={true}   /> 
       </div> 
          { this.state.OutFITProducts.map((prd, index) => {
             return(
               <div key={index} >
              
                <Card deleteProductFromOutFIT={this.deleteProductFromOutFIT}  haveClose={true} product = {prd} /> 
